...
function M.runFile()
    ...
    -- eg: "command $dir $path $name"
    local format = function(b)
        return string.gsub(string.gsub(string.gsub(b, "$dir", dir), "$path", path), "$name", name)
    end
    local run_cmd = {
        javascript = "node",
        typescript = "ts-node",
    ...
    elseif ft == "rust" then
        if existFile("Cargo.toml") then
            M.float("RUN", "cargo run")
        else
            M.float("RUN", "rustc % && ./%< && rm %<")
        end
    end
end
